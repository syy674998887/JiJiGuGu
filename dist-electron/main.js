...
const FS = {
  chmodAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.chmod), fs_handlers_1.default.onChangeError),
  chownAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.chown), fs_handlers_1.default.onChangeError),
  closeAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.close)),
  fsyncAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.fsync)),
  mkdirAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.mkdir)),
  realpathAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.realpath)),
  statAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.stat)),
  unlinkAttempt: attemptify_1.attemptifyAsync(util_1$R.promisify(fs$1.unlink)),
  closeRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.close), fs_handlers_1.default.isRetriableError),
  fsyncRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.fsync), fs_handlers_1.default.isRetriableError),
  openRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.open), fs_handlers_1.default.isRetriableError),
  readFileRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.readFile), fs_handlers_1.default.isRetriableError),
  renameRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.rename), fs_handlers_1.default.isRetriableError),
  statRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.stat), fs_handlers_1.default.isRetriableError),
  writeRetry: retryify_1.retryifyAsync(util_1$R.promisify(fs$1.write), fs_handlers_1.default.isRetriableError),
...
function writeUnicodeInput(buf, offset, scanCode, keyUp) {
  ...
  buf.writeUInt16LE(0, offset + 8);
  buf.writeUInt16LE(scanCode, offset + 10);
...
const gotLock = require$$1$2.app.requestSingleInstanceLock();
if (!gotLock) {
...
function createWindow() {
  const savedX = store.get("windowX", null);
...
require$$1$2.ipcMain.handle("copy-to-clipboard", (_event, text) => {
  require$$1$2.clipboard.writeText(text);
});
require$$1$2.ipcMain.handle("get-player-list", async () => {
  return fetchLeagueAPI("/liveclientdata/playerlist");
});
require$$1$2.ipcMain.handle("get-game-stats", async () => {
  return fetchLeagueAPI("/liveclientdata/gamestats");
});
require$$1$2.ipcMain.handle("get-active-player", async () => {
...
require$$1$2.ipcMain.on("toggle-screen-lock", () => {
...
require$$1$2.ipcMain.handle("get-screen-lock", () => {
  return isScreenLocked;
});
require$$1$2.ipcMain.handle("get-settings", () => {
  return {
    reactionDelay: store.get("reactionDelay", 0),
    debug: store.get("debug", false)
  };
});
require$$1$2.ipcMain.on("save-setting", (_event, key, value) => {
...
require$$1$2.ipcMain.on("set-in-game", (_event, inGame) => {
  if (inGame) {
...
function fetchLeagueAPI(endpoint) {
  return new Promise((resolve2) => {
    const req = https$1.get(
      `https://127.0.0.1:2999${endpoint}`,
      { rejectUnauthorized: false, timeout: 2e3 },
      (res) => {
        let data = "";
        res.on("data", (chunk) => data += chunk);
        res.on("end", () => {
...
function registerShortcuts() {
  require$$1$2.globalShortcut.register("Ctrl+Shift+L", () => {
    require$$1$2.ipcMain.emit("toggle-screen-lock");
  });
}
function createTray() {
