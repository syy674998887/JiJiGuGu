...
const STATUS_TITLES: Record<string, string> = {
    valid: 'Riot API Key: Valid',
    invalid: 'Riot API Key: Invalid / Expired',
    empty: 'Riot API Key: Not Set',
    checking: 'Riot API Key: Checking...',
}
    ...
    return (
        <div className="title-bar">
            <div className={`title-bar-drag ${isLocked ? 'no-drag' : ''}`}>
                ...
                <span
                    className={`api-status-dot status-${apiKeyStatus}`}
                    title={STATUS_TITLES[apiKeyStatus] || 'Unknown'}
                />
            </div>
